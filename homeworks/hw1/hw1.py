...
def hyperparameter_tuning():
    try:
        cv_results = pd.DataFrame(columns=['batch_size', 'lr', 'emb_dim', 'vocab_size', 'max_sent_length', 'optimizer', 'train_loss_hist', 'val_accuracies', 'max_accuracy'])
        ...
        for row in params.iterrows():
            ...
            batch_size, emb_dim, vocab_size, max_sent_length = int(row[1]['batch_size']), int(row[1]['emb_dim']), int(row[1]['vocab_size']), int(row[1]['max_sent_length'])
            lr = row[1]['lr']
            ...
            model = BagOfWords(len(id2token), emb_dim).to(DEVICE)
            criterion = nn.CrossEntropyLoss()
            optimizer = {'adam': torch.optim.Adam(model.parameters(), lr=lr), 'sgd': torch.optim.SGD(model.parameters(), lr=lr)}[row[1]['optimizer']]

            train_loss_history, val_accuracies = run_training(model, train_loader, val_loader, criterion, optimizer, N_EPOCHS)
            max_accuracy = np.max(val_accuracies)

            result = pd.DataFrame([[batch_size, lr, emb_dim, vocab_size, max_sent_length, train_loss_history, val_accuracies, max_accuracy]], columns=cv_results.columns)
